...
# this function returns the test accuarcies in a simple list, ordered as the datasets
def get_test_accuracies(information, extra_info=None):
  dataset_names = information.get_dataset_names()
...
class NASBench201API(object):
  ...
  def query_by_arch(self, arch, use_12epochs_result=False, return_test_accuracies=False, return_all_information=False):
    ...
    if arch_index in arch2infos:
      if return_all_information:
        return arch2infos[arch_index]
      if return_test_accuracies:
        accuracies = get_test_accuracies(arch2infos[arch_index])
        return accuracies
      else:
  ...
  def show(self, index: int = -1) -> None:
    ...
    if index < 0:  # show all architectures
      print(self)
      for i, idx in enumerate(self.evaluated_indexes):
        print('\n' + '-' * 10 + ' The ({:5d}/{:5d}) {:06d}-th architecture! '.format(i, len(self.evaluated_indexes),
                                                                                     idx) + '-' * 10)
