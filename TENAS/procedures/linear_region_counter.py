...
def get_datasets(name, root, input_size, cutout=-1):
    ...
    if name == 'cifar10' or name == 'cifar100':
        lists = [transforms.RandomCrop(input_size[1], padding=0), transforms.ToTensor(),
                 transforms.Normalize(mean, std), RandChannel(input_size[0])]
        ...
        test_transform = transforms.Compose(
            [transforms.Resize(40), transforms.CenterCrop(32), transforms.ToTensor(), normalize])
    else:
        raise TypeError("Unknow dataset : {:}".format(name))
...
class LinearRegionCount(object):
    ...
    def update2D(self, activations):
        ...
        self.activations[self.ptr:self.ptr + n_batch] = torch.sign(activations).half()  # after ReLU
        self.ptr += n_batch

    # @torch.no_grad()
    # def calc_LR(self):
    #    print('thingy size:', self.activations.half().size())
    #    res = torch.matmul(self.activations.half(), (1-self.activations).T.half())
    #    # res = torch.matmul(self.activations.T.half(), (1-self.activations).half())
    #    print('completed thingy size:', res.size())
    #    # res += torch.bmm(A.unsqueeze(0).expand_as(v), v)
    ...
    # @torch.no_grad()
    # def calc_LR(self):
    #    res = self.activations.half().clone()
    #    del self.activations 
    #    res = torch.matmul(res, (1-res).T)
    #    res = 1 - torch.sign(res)
    #    res = res.sum(1)
    #    res = 1. / res.float()
    #    self.n_LR = res.sum().item()
    ...
    # slightly memory friendlier reordering of the code: (Mark)
    @torch.no_grad()
    def calc_LR(self):
        # self.activations = self.activations.half()
        # self.activations = torch.matmul(self.activations.half(), (1-self.activations.half()).T)
        self.activations = torch.matmul(self.activations.half(), (1 - self.activations.half()).T)
        self.activations = 1 - torch.sign(self.activations)
        self.activations = self.activations.sum(1)
        self.activations = 1. / self.activations.float()
        self.n_LR = self.activations.sum().item()
        del self.activations
        self.activations = None
        torch.cuda.empty_cache()
...
class Linear_Region_Collector:
    def __init__(self, models=[], input_size=(64, 3, 32, 32), sample_batch=100, dataset='cifar100', data_path=None,
                 seed=0):
        self.models = []
    ...
    def reinit(self, models=None, input_size=None, sample_batch=None, seed=None):
        ...
        if input_size is not None or sample_batch is not None:
            ...
            if self.data_path is not None:
                ...
                self.train_loader = torch.utils.data.DataLoader(self.train_data, batch_size=self.input_size[0],
