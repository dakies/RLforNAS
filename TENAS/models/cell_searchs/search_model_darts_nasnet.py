    ...
    def genotype(self) -> Dict[Text, List]:
        def _parse(weights):
            ...
            for i in range(self._steps):
                selected_edges = []
                _edge_indice = sorted(range(i + 2), key=lambda x: -max(
                    weights[x][k] for k in range(len(weights[x])) if k != self.op_names.index('none')))[:2]
        ...
        return self.genotype2cmd(
            {'normal': gene_normal, 'normal_concat': list(range(2 + self._steps - self._multiplier, self._steps + 2)),
